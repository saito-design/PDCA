...
const TASKS_FILENAME = 'pdca-issues.json' // データファイル名は互換性のため維持
...
async function loadPdcaTasks(clientFolderId: string): Promise<PdcaIssue[]> {
  try {
    const result = await loadJsonFromFolder<PdcaIssue[]>(TASKS_FILENAME, clientFolderId)
    return result?.data || []
  } catch (error) {
...
export async function GET(
  ...
  try {
    ...
    // 企業の全タスクを取得
    const allTasks = await loadPdcaTasks(clientFolderId)
    const filtered = allTasks.filter((t) => t.client_id === clientId)
    ...
    console.error('Get all pdca tasks error:', error)
    return NextResponse.json(
